...
if __name__ == '__main__':
    ...
    val_data_loader = DataLoader(
        val_train, batch_size=val_batch_size, num_workers=5, shuffle=False, pin_memory=False)
    ...
    scheduler = lr_scheduler.MultiStepLR(optimizer,
                                         milestones=[
                                             15, 29, 43, 53, 65, 80, 90, 100, 110, 130, 150, 170, 180, 190],
                                         gamma=0.5)

    model = nn.DataParallel(model).cuda()

    wandb.init(project="xview2_no1_solution", tags=["low resolution", "loc"], notes=f"seed: {seed}",
