...
class PathHandler:
    def __init__(self, pred_dir: Path, targ_dir: Path, img_id: str, disaster_type: str):
        """
        Args:
            ...
            disaster_type (str) : # either 'test' or 'hold'. Most likely 'test' unless you have access to holdout set
        """
        ...
        # assert disaster_type in ['test', 'hold'], f"test_hold '{disaster_type}' was not one of 'test' or 'hold'"

        # Changed paths here
        self.lp = pred_dir / f"{disaster_type}_{img_id}_localization_disaster_prediction.png"  # localization prediction
        self.dp = pred_dir / f"{disaster_type}_{img_id}_damage_disaster_prediction.png"  # damage prediction
        self.lt = targ_dir / f"{disaster_type}_{img_id}_pre_disaster_target.png"  # localization target
        self.dt = targ_dir / f"{disaster_type}_{img_id}_post_disaster_target.png"  # damage target
...
class XviewMetrics:
    ...
    def get_path_handlers(self):
        ...
        for path in self.targ_dir.glob('*.png'):
            disaster_type, img_id, pre_post, _, target = path.name.rstrip('.png').split('_')
            assert pre_post in ['pre', 'post'], f"target filenames must have 'pre' or 'post' in filename, got {path}"
            ...
            if pre_post == 'pre':  # localization or damage is fine here
                self.path_handlers.append(PathHandler(self.pred_dir, self.targ_dir, img_id, disaster_type))
...
if __name__ == '__main__':
    output_root = "/local_storage/users/paulbp/xview2/"
    submissions_dir = output_root + "predictions/winner/submission_indiv_single_rot_use154_False/"
    targets_dir = "/local_storage/datasets/sgerard/xview2/test/targets/"

    metrics_dics = []
    for pred_dir in glob.glob(str(Path(submissions_dir) / "*")):
        if not Path(pred_dir).is_dir():
            continue
        output_json = str(Path(pred_dir) / "metrics.json")
        metrics_dic = XviewMetrics.compute_score(pred_dir, targets_dir, output_json)
        # if "tuned" is in the name of the directory it means the name of the directory
        # is the name of the exact model (i.e. seed + architecture) 
        # and not a ensemble of different models with the same architecture
        metrics_dic["ensemble"] = not ("tuned" in pred_dir)
        metrics_dic["model_name"] = pred_dir.split("/")[-1]
        metrics_dics.append(metrics_dic)

    combined_metrics_path = str(Path(submissions_dir) / "combined_metrics.csv")
    pd.DataFrame(metrics_dics).to_csv(combined_metrics_path, index=False)
