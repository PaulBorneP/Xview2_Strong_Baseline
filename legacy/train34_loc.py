    ...
    def __getitem__(self, idx):
        ...
        if random.random() > 0.3:
            crop_size = random.randint(
                int(input_shape[0] / 1.2), int(input_shape[0] / 0.8))
...
if __name__ == '__main__':
    ...
    wandb.init(project="xview2_no1_solution", tags=["resnet34", "loc"], notes=f"seed: {seed}",
               dir="/local_storage/users/paulbp/xview2/logs/")
    wandb.watch(model, log="all")

    seg_loss = ComboLoss({'dice': 1.0, 'focal': 10.0},
