...
models_folder = '/local_storage/users/paulbp/xview2/weights/low_augment/'

loc_folder = '/local_storage/users/paulbp/xview2/predictions/low_augment/pred_loc_val/'
    ...
    def __getitem__(self, idx):
        ...
        if random.random() > 0.1:
            crop_size = random.randint(
                int(input_shape[0] / 1.15), int(input_shape[0] / 0.85))
        ...
        if crop_size != input_shape[0]:
            ...
            msk0 = cv2.resize(msk0, input_shape,
                              interpolation=cv2.INTER_LINEAR)
            msk1 = cv2.resize(msk1, input_shape,
                              interpolation=cv2.INTER_LINEAR)
            msk2 = cv2.resize(msk2, input_shape,
                              interpolation=cv2.INTER_LINEAR)
            msk3 = cv2.resize(msk3, input_shape,
                              interpolation=cv2.INTER_LINEAR)
            msk4 = cv2.resize(msk4, input_shape,
                              interpolation=cv2.INTER_LINEAR)
        """
...
class ValData(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        sample = {'img': img, 'msk': msk,
                  'lbl_msk': lbl_msk, 'fn': fn, 'msk_loc': msk_loc}
        return sample
...
def validate(net, data_loader):
    ...
    with torch.no_grad():
        for i, sample in enumerate(tqdm(data_loader)):
            ...
            msk_pred = msk_loc
            msk_damage_pred = torch.softmax(
                out, dim=1).cpu().numpy()[:, 1:, ...]
...
def train_epoch(current_epoch, seg_loss, ce_loss, model, optimizer, scheduler, train_data_loader):
    ...
    for i, sample in enumerate(iterator):
        ...
        loss = 0.1 * loss0 + 0.1 * loss1 + 0.3 * \
            loss2 + 0.3 * loss3 + 0.2 * loss4 + loss5 * 11
...
if __name__ == '__main__':
    ...
    for epoch in range(20):
        train_epoch(epoch, seg_loss, ce_loss, model,
